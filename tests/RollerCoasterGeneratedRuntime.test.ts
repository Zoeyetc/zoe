...
import { planRollerCoasterTrack } from '../apps/zland/src/rides/roller-coaster/trackPlan.ts';
import { createRollerCoasterTrack } from '../apps/zland/src/rides/roller-coaster/trackMap.ts';
import { createRollerCoasterSimulation, ROLLER_MAX_ACCELERATION, ROLLER_MAX_VELOCITY } from '../apps/zland/src/rides/roller-coaster/simulation.ts';
import { rollerCoasterToPhysicsWake } from '../apps/zland/src/physics/adapters/rollerCoaster.ts';

const generated = createRollerCoasterTrack(milestoneSevenZlandAudioMap, planRollerCoasterTrack);
...
test('fixture generation is deterministic across instances',()=>{const other=createRollerCoasterTrack(milestoneSevenZlandAudioMap,planRollerCoasterTrack);assert.equal(generated.map.id,other.map.id);});
...
test('experience replaces track only in source activation paths',()=>{const source=readFileSync(new URL('../apps/zland/src/createExperience.ts',import.meta.url),'utf8');assert.match(source,/activateRealAudio[\s\S]*createRollerCoasterTrack/);assert.doesNotMatch(source,/const read = \(\) => \{[\s\S]{0,500}createRollerCoasterTrack/);});
