...
import { DoodlePath } from '../../experience/doodle/DoodleSvg';

const PIVOT_X = 280;
const PIVOT_Y = 46;
const ARM_LENGTH = 176;

export function PirateShipView({ state, districtFit = false, doodle = false }: {
  ...
  return <section aria-labelledby="pirate-ship-heading">
    ...
    <div className="pirate-stage">
      <svg className="pirate-machine" viewBox="0 0 560 300"
        ...
        {doodle ? <DoodlePath className="pirate-support" d={`M90 282 L${PIVOT_X} ${PIVOT_Y} L470 282 M70 282 H490`}
          seed="pirate-support" roughness={0.8} secondary /> : <path className="pirate-support" d={`M90 282 L${PIVOT_X} ${PIVOT_Y} L470 282 M70 282 H490`} />}
        ...
        <g className="pirate-swing" transform={`translate(${PIVOT_X} ${PIVOT_Y}) rotate(${degrees})`}>
          {doodle ? <DoodlePath className="pirate-arm" d={`M0 0 L-94 ${ARM_LENGTH} M0 0 L94 ${ARM_LENGTH}`}
            seed="pirate-arm" roughness={0.55} /> : <path className="pirate-arm" d={`M0 0 L-94 ${ARM_LENGTH} M0 0 L94 ${ARM_LENGTH}`} />}
          {doodle ? <DoodlePath className="pirate-carrier" d={`M-114 ${ARM_LENGTH - 6} Q0 ${ARM_LENGTH + 42} 114 ${ARM_LENGTH - 6} L96 ${ARM_LENGTH + 28} H-96 Z`}
            seed="pirate-carrier" roughness={0.7} /> : <path className="pirate-carrier" d={`M-114 ${ARM_LENGTH - 6} Q0 ${ARM_LENGTH + 42} 114 ${ARM_LENGTH - 6} L96 ${ARM_LENGTH + 28} H-96 Z`} />}
