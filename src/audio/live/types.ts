...
export type LiveInputStatus =
  | 'NOT_REQUESTED' | 'REQUESTING' | 'LIVE' | 'DENIED' | 'NO_DEVICE'
  | 'DEVICE_LOST' | 'STOPPED' | 'ERROR';
...
export type LiveAnalysisUpdate = Readonly<{
  map: AudioMap; transport: TransportState; events: readonly AudioEvent[]; state: LiveInputState;
}>;
...
const listener = (state: LiveListenerState, elapsed: number, required: number): LiveListenerStatus =>
  ({ state, elapsed, required });
export function emptyLiveListeners(): LiveInputState['listeners'] {
  return {
    signal: listener('WARMING_UP', 0, 0.05), melody: listener('WARMING_UP', 0, 2),
    rhythm: listener('WARMING_UP', 0, 4), percussion: listener('WARMING_UP', 0, 0.3),
    harmony: listener('WARMING_UP', 0, 2), tonalCenter: listener('WARMING_UP', 0, 8),
...
export const INITIAL_LIVE_INPUT_STATE: LiveInputState = Object.freeze({
  ...
  analysisCadence: LIVE_ANALYSIS_CADENCE_SECONDS, droppedAnalysisRequests: 0,
  lastAnalysisLatency: null, listeners: emptyLiveListeners(), error: null,
});
