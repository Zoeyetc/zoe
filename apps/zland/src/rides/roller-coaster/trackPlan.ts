...
const energyAt = (map: ZlandAudioMap, time: number) => {
...
export function createMacroEnergy(map: ZlandAudioMap): readonly MacroEnergyPoint[] {
...
export function createSafeTrackPlan(map: Pick<ZlandAudioMap, 'id' | 'duration'>, reason = 'structure evidence unavailable'): TrackPlan {
...
const sourceSegments = (map: ZlandAudioMap): { source: TrackEvidenceSource; segments: readonly PlannerSegment[] } | null => {
...
function makeEvidence(map: ZlandAudioMap, source: TrackEvidenceSource, segment: PlannerSegment, previousEnergy: number, nextEnergy: number) {
...
export function planRollerCoasterTrack(map: ZlandAudioMap): TrackPlan {
