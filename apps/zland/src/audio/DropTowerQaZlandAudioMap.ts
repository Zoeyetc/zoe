import { milestoneSevenZlandAudioMap } from './ZlandAudioMaps.ts';
import type { StructureAnalysis, StructureBoundary, StructureSegment } from '@computational-listening/engine';
...
export function createDropTowerQaZlandAudioMap(scenario: DropTowerQaScenario): ZlandAudioMap {
  const unavailable = scenario === 'unavailable';
  return {
    ...milestoneSevenZlandAudioMap,
    id: `drop-tower-qa-${scenario}`,
    duration: 16,
    capabilities: { ...milestoneSevenZlandAudioMap.capabilities, structure: !unavailable },
    structure: null,
    drops: null,
...
export function resolveDropTowerQaZlandAudioMap(value: string | null): ZlandAudioMap | null {
  return value === 'prepared-release' || value === 'weak-boundary'
    || value === 'unprepared-boundary' || value === 'nearby-boundaries' || value === 'unavailable'
    ? createDropTowerQaZlandAudioMap(value) : null;
}
