...
import { decodeLocalAudioFile, pcmFromAudioBuffer } from '@computational-listening/audio-source-browser';
import { analyzeZlandPcmAudioAsync } from './analyzeZlandAudio.ts';
import { createLatestRequestGuard } from './requestGuard.ts';
  ...
  return {
    ...
    async prepare(file: File): Promise<PreparedRealAudio | null> {
      ...
      try {
        ...
        const map = await analyzeZlandPcmAudioAsync(pcmFromAudioBuffer(buffer), {
